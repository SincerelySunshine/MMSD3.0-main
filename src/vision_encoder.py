...
class VisionEncoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        model_name = config.get_vision_model_name()
        model_source = config.get_model_load_path(model_name)
        load_kwargs = config.get_hf_load_kwargs()
        ...
        if self.encoder_type == "clip":
            ...
            with warnings.catch_warnings():
                ...
                self.vision_model = CLIPVisionModel.from_pretrained(model_source, **load_kwargs)
            print(f"Loading CLIP vision model from: {model_source}")
        elif self.encoder_type == "vit":
            self.vision_model = ViTModel.from_pretrained(model_source, **load_kwargs)
            print(f"Loading ViT model from: {model_source}")
        else:
