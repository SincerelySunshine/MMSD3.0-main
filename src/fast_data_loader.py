...
class FastSarcasmDataset(Dataset):
    ...
    def __init__(self, config, mode='train'):
        ...
        hf_load_kwargs = config.get_hf_load_kwargs()
        ...
        if config.text_encoder_type == 'bert':
            ...
            tokenizer_source = config.get_model_load_path(tokenizer_name)
            self.tokenizer = BertTokenizer.from_pretrained(tokenizer_source, **hf_load_kwargs)
        else:  # roberta
            # Use RoBERTa model specified in config
            tokenizer_name = config.get_text_model_name()
            tokenizer_source = config.get_model_load_path(tokenizer_name)
            self.tokenizer = RobertaTokenizer.from_pretrained(tokenizer_source, **hf_load_kwargs)
    ...
    def _load_ocr_data(self):
        ...
        ocr_file = os.path.join(self.config.project_root, 'ocr_results', 'mmsd3_all_ocr.json')
    ...
    def _load_mmsd3_data(self):
        ...
        for item in data:
            file_id = str(item['id'])
            # Open-source Twitter samples may not include original post text.
            text_arr[file_id] = item.get('text', "")
            ...
            if item['images']:
                ...
                for img_path_str in item['images']:
                    full_path = self._resolve_image_path(img_path_str)
    ...
    def _resolve_image_path(self, img_path_str):
        """
        Resolve image path from dataset json to real local image path.
        For MMSD3 entries like './images/xxx.jpg', this maps to config.image_path/xxx.jpg.
        """
        raw_path = str(img_path_str).strip().replace("\\", "/")
        ...
        marker_index = raw_path.lower().find("images/")
        if marker_index != -1:
            raw_path = raw_path[marker_index + len("images/"):]

        return os.path.normpath(os.path.join(self.config.image_path, raw_path))
